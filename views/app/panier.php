...
?>
    ...
    <?php if (!empty($_SESSION['membre'])) : ?>
        <a href="<?= BASE_PATH . 'produits/commande' ?>" class="btn btn-secondary">Passer la commande</a>
    <?php else : ?>
        <a href="<?= BASE_PATH . 'user/connexion?commande=true' ?>" class="btn btn-secondary">Connectez-vous pour passer la commande</a>
    <?php endif; ?>
